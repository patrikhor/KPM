...
main(int argc, char* argv[])
{
    ...
    // Traffic parameters:
    uint32_t udpPacketSizeULL = 100; //Ultra low latency
    uint32_t udpPacketSizeBe = 1252; //Best effort (voice)
    uint32_t lambdaULL = 10000; // 1/interval
    ...
    // NR parameters. 
    uint16_t numerologyBwp1 = 4; //Numerology for BWP 1
    double centralFrequencyBand1 = 28e9;
    double bandwidthBand1 = 200e6;
    uint16_t numerologyBwp2 = 2; //Numerology for BWP 2
    double centralFrequencyBand2 = 28.6e9;
    double bandwidthBand2 = 200e6;
    ...
    const uint8_t numCcPerBand = 2; // in this example, both bands have a single CC
    ...
    std::vector<OperationBandInfo*> oBIVec;
    oBIVec.push_back(&band1);
    oBIVec.push_back(&band2);
    ccBwpCreator.PlotNrCaBwpConfiguration(oBIVec,"5G-nr-1.plt");
    ...
    serverApps2.Add(ulPacketSinkVoice.Install(remoteHost));
    ...
    ulClientVoice.SetAttribute("RemoteAddress",AddressValue(remoteHostAddress));

    //The bearer that will carry voice traffic
    EpsBearer voiceBearer2(EpsBearer::GBR_CONV_VOICE);

    //Filter for voice trafic
    Ptr<EpcTft> voiceTft2 = Create<EpcTft>();
    EpcTft::PacketFilter ulpfVoice;
    ulpfVoice.localPortStart = ulPortVoice;
    ulpfVoice.localPortEnd = ulPortVoice;
    voiceTft2->Add(ulpfVoice);
    ...
    for (uint32_t i = 0; i < ueVoiceContainer.GetN(); ++i)
    {
        ...
        clientApps2.Add(ulClientVoice.Install(ueVoiceContainer.Get(i)));

        // Activate a dedicated bearer for the traffic type
        nrHelper->ActivateDedicatedEpsBearer(ueDevice, voiceBearer2, voiceTft2);
    }



    //   nezapomeň přidat clientapp2 a serverApps2 start/STOP                           !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    serverApps2.Start(udpAppStartTime);
    clientApps.Start(udpAppStartTime);
    clientApps2.Start(udpAppStartTime);
    ...
    AnimationInterface animation("5g-nr.xml");
