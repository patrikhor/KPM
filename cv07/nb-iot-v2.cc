...
NS_LOG_COMPONENT_DEFINE ("LenaNb");
...
main (int argc, char *argv[])
{
  ...
  uint16_t num_ues_app_a = 1;
  uint16_t num_ues_app_b = 2;
  uint16_t num_ues_app_c = 2;
  uint16_t packetsize_app_a = 49; // 32 Bytes 5G mMTC payload + 4 Bytes CoAP Header + 13 Bytes DTLS Header // UDP Header and IP Header  are added by NS-3
  uint16_t packetsize_app_b = 49;
  uint16_t packetsize_app_c = 49;
  ...
  cmd.AddValue ("simName", "Simulation name", simName);
  ...
  positionAlloc->Add (Vector (cellsize/2, cellsize/2, 50)); // Place our single eNb right in the center of the cell
  ...
  */
  ...
  for (uint32_t j = 0; j<3; j++){ // Pre-Run, Run, Post-Run.
    ...
    pos_a.Set ("X", StringValue (std::to_string(cellsize/2))); // x center
    pos_a.Set ("Y", StringValue (std::to_string(cellsize/2))); // y center
    pos_a.Set ("Z", DoubleValue (1.5)); // elevation of the disc
    pos_a.Set ("rho", DoubleValue (cellsize/2)); // disc radius
  ...
  logdir += "_";
  logdir += std::to_string(std::round(simTime.GetSeconds()));
  logdir += "_";
  ...
  AnimationInterface animation("nb-iot.xml");
  animation.UpdateNodeDescription(pgw, "PGW");
  animation.UpdateNodeDescription(remoteHost, "RemoteHost");
  ...
  monitor->SerializeToXmlFile("nb-iot.flowmon", true, true);
